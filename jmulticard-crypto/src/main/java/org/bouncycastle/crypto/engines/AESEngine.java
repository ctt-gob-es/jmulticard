...
private static final int[] Tinv0 =
    ...
    private static int shift(final int r, final int shift)
    {
    ...
    private static int FFmulX(final int x)
    {
        return (x & m2) << 1 ^ ((x & m1) >>> 7) * m3;
    }

    private static int FFmulX2(final int x)
    {
        final int t0  = (x & m5) << 2;
        int t1  = x & m4;
            t1 ^= t1 >>> 1;
    ...
    */
    ...
    private static int inv_mcol(final int x)
    {
        int t0, t1;
        t0  = x;
        t1  = t0 ^ shift(t0, 8);
    ...
    private static int subWord(final int x)
    {
     ...
     */
    private int[][] generateWorkingKey(final byte[] key, final boolean forEncryption)
    {
        ...
        final int KC = keyLen >>> 2;
        ROUNDS = KC + 6;  // This is not always true for the generalized Rijndael that allows larger block sizes
        final int[][] W = new int[ROUNDS+1][4];   // 4 words in a block
        ...
        case 4:
        {
            ...
            for (int i = 1; i <= 10; ++i)
            {
                final int colx = subWord(shift(col3, 8)) ^ rcon[i - 1];
    ...
    private void encryptBlock(final byte[] in, final int inOff, final byte[] out, final int outOff, final int[][] KW)
    {
    ...
    private void decryptBlock(final byte[] in, final int inOff, final byte[] out, final int outOff, final int[][] KW)
    {
        int C0 = Pack.littleEndianToInt(in, inOff +  0);
        int C1 = Pack.littleEndianToInt(in, inOff +  4);
        int C2 = Pack.littleEndianToInt(in, inOff +  8);
