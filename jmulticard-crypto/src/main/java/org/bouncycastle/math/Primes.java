...
public abstract class Primes {
    ...
    {
        static MROutput probablyPrime()
        {
            return new MROutput(false, null);
        }

        static MROutput provablyCompositeWithFactor(final BigInteger factor)
        {
            return new MROutput(true, factor);
        }

        static MROutput provablyCompositeNotPrimePower()
        {
            return new MROutput(true, null);
        }

        private final boolean provablyComposite;
    ...
    public static class STOutput
    {
        ...
        private final byte[] primeSeed;
        private final int primeGenCounter;
     ...
     */
    public static STOutput generateSTRandomPrime(final Digest hash, final int length, final byte[] inputSeed)
    {
        if (hash == null)
        {
            throw new IllegalArgumentException("'hash' cannot be null"); //$NON-NLS-1$
        }
        if (length < 2)
        {
            throw new IllegalArgumentException("'length' must be >= 2"); //$NON-NLS-1$
        }
        if (inputSeed == null || inputSeed.length == 0)
        {
            throw new IllegalArgumentException("'inputSeed' cannot be null or empty"); //$NON-NLS-1$
        }
     ...
     */
    public static MROutput enhancedMRProbablePrimeTest(final BigInteger candidate, final SecureRandom random, final int iterations)
    {
        checkCandidate(candidate, "candidate"); //$NON-NLS-1$

        if (random == null)
        {
            throw new IllegalArgumentException("'random' cannot be null"); //$NON-NLS-1$
        }
        if (iterations < 1)
        {
            throw new IllegalArgumentException("'iterations' must be > 0"); //$NON-NLS-1$
        }
        ...
        final BigInteger w = candidate;
        final BigInteger wSubOne = candidate.subtract(ONE);
        final BigInteger wSubTwo = candidate.subtract(TWO);

        final int a = wSubOne.getLowestSetBit();
        final BigInteger m = wSubOne.shiftRight(a);

        for (int i = 0; i < iterations; ++i)
        {
            final BigInteger b = BigIntegers.createRandomInRange(TWO, wSubTwo, random);
     ...
     */
    public static boolean hasAnySmallFactors(final BigInteger candidate)
    {
        checkCandidate(candidate, "candidate"); //$NON-NLS-1$
     ...
     *         that were performed).
     */
    public static boolean isMRProbablePrime(final BigInteger candidate, final SecureRandom random, final int iterations)
    {
        checkCandidate(candidate, "candidate"); //$NON-NLS-1$
     ...
     */
    public static boolean isMRProbablePrimeToBase(final BigInteger candidate, final BigInteger base)
    {
        checkCandidate(candidate, "candidate"); //$NON-NLS-1$
        checkCandidate(base, "base"); //$NON-NLS-1$

        if (base.compareTo(candidate.subtract(ONE)) >= 0)
        {
            throw new IllegalArgumentException("'base' must be < ('candidate' - 1)"); //$NON-NLS-1$
        }
    ...
    private static void checkCandidate(final BigInteger n, final String name)
    {
        if (n == null || n.signum() < 1 || n.bitLength() < 2)
        {
            throw new IllegalArgumentException("'" + name + "' must be non-null and >= 2"); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }

    private static boolean implHasAnySmallFactors(final BigInteger x)
    {
        /*
    ...
    private static boolean implMRProbablePrimeToBase(final BigInteger w, final BigInteger wSubOne, final BigInteger m, final int a, final BigInteger b)
    {
        BigInteger z = b.modPow(m, w);
    ...
    private static STOutput implSTRandomPrime(final Digest d, final int length, byte[] primeSeed)
    {
        final int dLen = d.getDigestSize();

        if (length < 33)
        {
            int primeGenCounter = 0;

            final byte[] c0 = new byte[dLen];
            final byte[] c1 = new byte[dLen];
            ...
            for (;;)
            {
                ...
                {
                    throw new IllegalStateException("Too many iterations in Shawe-Taylor Random_Prime Routine"); //$NON-NLS-1$
                }
            }
        }

        final STOutput rec = implSTRandomPrime(d, (length + 3) / 2, primeSeed);

        final BigInteger c0 = rec.getPrime();
        primeSeed = rec.getPrimeSeed();
        int primeGenCounter = rec.getPrimeGenCounter();

        final int outlen = 8 * dLen;
        final int iterations = (length - 1) / outlen;

        final int oldCounter = primeGenCounter;

        BigInteger x = hashGen(d, primeSeed, iterations + 1);
        x = x.mod(ONE.shiftLeft(length - 1)).setBit(length - 1);

        final BigInteger c0x2 = c0.shiftLeft(1);
        ...
        for (;;)
        {
            ...
            if (!implHasAnySmallFactors(c))
            {
                ...
                final BigInteger z = a.modPow(tx2, c);
    ...
    private static int extract32(final byte[] bs)
    {
        int result = 0;

        final int count = Math.min(4, bs.length);
        for (int i = 0; i < count; ++i)
        {
            final int b = bs[bs.length - (i + 1)] & 0xFF;
            result |= b << 8 * i;
        }
    ...
    private static void hash(final Digest d, final byte[] input, final byte[] output, final int outPos)
    {
        d.update(input, 0, input.length);
        d.doFinal(output, outPos);
    }

    private static BigInteger hashGen(final Digest d, final byte[] seed, final int count)
    {
        final int dLen = d.getDigestSize();
        int pos = count * dLen;
    ...
    private static void inc(final byte[] seed, int c)
    {
        int pos = seed.length;
        while (c > 0 && --pos >= 0)
        {
    ...
    private static boolean isPrime32(final long x)
    {
        if (x >>> 32 != 0L)
        {
            throw new IllegalArgumentException("Size limit exceeded"); //$NON-NLS-1$
        }
        ...
        final long[] ds = { 1L, 7L, 11L, 13L, 17L, 19L, 23L, 29L };
        long base = 0L;
        for (int pos = 1;; pos = 0)
        {
            /*
