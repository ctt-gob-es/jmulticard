...
class HT
{
    ...
    byte[] sign(final byte[] M, long idx_tree, int idx_leaf)
    {
        // init
        final ADRS adrs = new ADRS();
        // sign
        ...
        final SIG_XMSS[] SIG_HT = new SIG_XMSS[engine.D];
        SIG_HT[0] = SIG_tmp;
        ...
        final byte[][] totSigs = new byte[SIG_HT.length][];
    ...
    byte[] xmss_PKgen(final byte[] skSeed, final byte[] pkSeed, final ADRS adrs)
    {
    ...
    byte[] xmss_pkFromSig(final int idx, final SIG_XMSS sig_xmss, final byte[] M, final byte[] pkSeed, final ADRS paramAdrs)
    {
        final ADRS adrs = new ADRS(paramAdrs);

        // compute WOTS+ pk from WOTS+ sig
        adrs.setType(ADRS.WOTS_HASH);
        adrs.setKeyPairAddress(idx);
        final byte[] sig = sig_xmss.getWOTSSig();
        final byte[][] AUTH = sig_xmss.getXMSSAUTH();
    ...
    SIG_XMSS xmss_sign(final byte[] M, final byte[] skSeed, final int idx, final byte[] pkSeed, ADRS adrs)
    {
        final byte[][] AUTH = new byte[engine.H_PRIME][];

        // build authentication path
        for (int j = 0; j < engine.H_PRIME; j++)
        {
            final int k = idx / (1 << j) ^ 1;
        ...
        final byte[] sig = wots.sign(M, skSeed, pkSeed, adrs);
...
//    # Output: Boolean
    public boolean verify(final byte[] M, final SIG_XMSS[] sig_ht, final byte[] pkSeed, long idx_tree, int idx_leaf, final byte[] PK_HT)
    {
        // init
        final ADRS adrs = new ADRS();
        // verify
