...
public class BcPaceChannelHelper extends PaceChannelHelper {
	...
	/** Construye una clase de utilidades para el establecimiento de un canal PACE.
	...
	public SecureMessaging openPaceChannel(final byte cla,
		...
		final byte[] kenc;
		try {
			kenc = padAndDigest(secretK, KENC_PADDING, DigestAlgorithm.SHA1, paceAlgorithm.getKeyLength());
		}
		catch (final IOException e) {
			throw new PaceException("Error obteniendo el 'kenc' a partir del CAN/MRZ", e); //$NON-NLS-1$
		}

		// Calcular kmac = SHA-x(k || 00000002);
		final byte[] kmac;
		try {
			kmac = padAndDigest(secretK, KMAC_PADDING, DigestAlgorithm.SHA1, paceAlgorithm.getKeyLength());
		}
		catch (final IOException e) {
			throw new PaceException("Error obteniendo el 'kmac' a partir del CAN", e); //$NON-NLS-1$
		}
